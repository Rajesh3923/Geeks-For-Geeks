...
class Solution{
     public static void createTree(Node root0, ArrayList<Integer> v) {
        ...
        int index = 1; // Start from the second element since root0 is already the first element
        while (!queue.isEmpty() && index < v.size()) {
            Node currentNode = queue.poll();
            
            // Assign the left child
            if (index < v.size()) {
                currentNode.left = new Node(v.get(index));
            ...
            if (index < v.size()) {
                currentNode.right = new Node(v.get(index));
